...
class Config:
    ...
    array_dir: Path = Path(__file__).parent  # EHT2017.txt lives next to this file
...
def generate(cfg: Config):
    ...
    for i, obs_frame in tqdm(list(enumerate(obs_frames)), desc="Generating frames"):
        ...
        if cfg.include_cp:
            cp_target, cp_sigma, cp_tri = closure_data(target, r["amp_sigma"], obs.data[: len(target)])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--movie-name", default=None, help="Tag used in the output dir name (default: movie file stem)")
    parser.add_argument("--out", default="./data", help="Output root directory")
    parser.add_argument("--array", default="EHT2017", help="Array name (expects <array>.txt in --array-dir)")
    parser.add_argument("--array-dir", default=str(Path(__file__).parent), help="Directory containing the array .txt file")
    parser.add_argument("--noise", type=float, default=0.04, help="Fractional systematic noise")
    parser.add_argument("--scale-factor", type=int, default=5, help="Downsampling factor for the model grid")
